...
local function getAvgSessionMinutes(stats)
    ...
    if avg_minutes >= 60 then
        local hours = math.floor(avg_minutes / 60)
        local minutes = math.floor((avg_minutes % 60) + 0.5)
        if minutes == 60 then
            hours = hours + 1
            minutes = 0
        end
...
local function getSecondsPerPage(stats)
    local total_pages, total_duration = getValidSessionTotals(stats)
    if total_pages <= 0 then
        return "-"
    end
    local sec_per_page = total_duration / total_pages
    return string.format("%.1f %s", sec_per_page, _("sec/page"))
end
...
function ReadingStatsTable:buildContent()
    ...
    local sec_per_page = getSecondsPerPage(all_stats)
    ...
    local meta2 = TextWidget:new{
        text = string.format("%s   ·   %s: %s   ·   %s",
            visible_speed, _("Daily avg"), avg_session_minutes, sec_per_page),
        face = self.fonts.meta,
    }
